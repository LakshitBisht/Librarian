...
#include "ui_signup_member.h"

SignUp_Member::SignUp_Member(QWidget *parent) : QMainWindow(parent),
                                                ui(new Ui::SignUp_Member)
{
    ui->setupUi(this);
...
//Deleting the Form instance when window is closed
    ...
    nativeParentWidget()->setWindowTitle("Librarian - Member SignUp");
...
void SignUp_Member::on_pushButton_memLogin_clicked()
{
...
void SignUp_Member::on_pushButton_memSubmit_clicked()
{
//taking values from the input fields
    ...
    UID = ui->lineEdit_memUID->text();
    password = ui->lineEdit_memPassword->text();
...
//if all conditions are satisfied
    ...
    //if query executed
        ...
        //if no match found
            ...
            //preparing the query
                query.prepare("insert into Members (First_Name, Last_Name, Gender, Profession, UID, Password, Security_Ques, Answer, Contact_No) "
            ...
            //if query executed
                if (query.exec())
                {
                    query.prepare("insert into Registration_Log (Registration_Date, Username_UID, Name, Account) "
                                  "values('" + QDate::currentDate().toString() + "','" + UID + "','" + fName + " " + lName + "','Member')");
                    if (query.exec())
                    {
                        //displaying system message
                            QMessageBox::about(this, "Sign Up Successful!", "Account has been registered as a Member ");
        ...
        //if match found
            ...
            //displaying system warning
                QMessageBox::warning(this, "Sign Up Unsuccessful!", "Account already exists! ");
            }
        }
...
void SignUp_Member::on_lineEdit_fName_textChanged(const QString &arg1)
{
    if(arg1 == "")
...
void SignUp_Member::on_lineEdit_lName_textChanged(const QString &arg1)
{
    if(arg1 == "")
...
void SignUp_Member::on_comboBox_gender_currentIndexChanged(int index)
{
    if(index == -1)
...
void SignUp_Member::on_comboBox_profession_currentIndexChanged(int index)
{
    if(index == -1)
...
void SignUp_Member::on_lineEdit_memUID_textChanged(const QString &arg1)
{
    if(arg1 == "")
            ui->lineEdit_memUID->setStyleSheet("color: #717072;");
        else
            ui->lineEdit_memUID->setStyleSheet("color:white;");
}


void SignUp_Member::on_lineEdit_memPassword_textChanged(const QString &arg1)
{
    if(arg1 == "")
    {
        ui->lineEdit_memPassword->setStyleSheet("color: #717072;");
        ui->label_passStrength->hide();
    }
    else
    {
        ui->lineEdit_memPassword->setStyleSheet("color:white;");
...
void SignUp_Member::on_comboBox_question_currentIndexChanged(int index)
{
    if(index == -1)
...
void SignUp_Member::on_lineEdit_answer_textChanged(const QString &arg1)
{
    if(arg1 == "")
...
void SignUp_Member::on_lineEdit_phone_textChanged(const QString &arg1)
{
    if(arg1 == "")
